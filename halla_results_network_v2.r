...
# Add Edge Type attribute and filter by association dependent on correlation type
# mox_halla2 <- mox_halla2 %>% 
#     mutate(Edge_Type = paste(Source_Type, Target_Type, sep = "|")) %>% 
#     filter(case_when(
#                     Edge_Type %in% c("dna_viral|dna_pfam", 
#                                      "amp_asv|dna_pfam",
#                                      "dna_taxonomy|dna_pfam", 
#                                      "rna_taxonomy_bkn|rna_pfam", 
#                                      "dna_pfam|cfu_culture"
#                                      ) ~ abs(association) > 0.6,
#                     # str_detect(Edge_Type, "cfu") ~ abs(association) > .2,
#                     # str_detect(Edge_Type, "abx_medication") ~ abs(association) > .2,
#                     str_detect(Edge_Type, "metadata") ~ association > .2,
#                     .default = abs(association) > .5))
...
####################################
# 
# tmp <- mox_clusters %>% filter(Source_Type != Target_Type) %>%  mutate(cluster_node = paste(Cluster, Source_Type, Target_Type, sep = "_"),
#                                cluster_type = paste(Source_Type, Target_Type, sep = "|"))
# tmpx <- tmp %>% dplyr::select(cluster_node, Cluster,cluster_type, Source, Source_Type, Cluster_FDR) %>% rename(feature=Source, type=Source_Type)
# tmpy <- tmp %>% dplyr::select(cluster_node, Cluster, cluster_type, Target, Target_Type, Cluster_FDR) %>% rename(feature=Target, type=Target_Type)
# 
# cluster_propogation <- rbind(tmpx,tmpy) %>% distinct()
# write_tsv(cluster_propogation, 'halla/cluster_propogation_table.tsv')
# cluster_propogation %>% filter(Cluster_FDR !=0)
# cluster_propogation_sub<- cluster_propogation %>% filter(Cluster < 100)
# write_tsv(cluster_propogation_sub, 'halla/cluster_propogation_sub_table.tsv')
# write
